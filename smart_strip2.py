...
# Regex for outfit: ALWAYS matches #ifdef MUDSKIP_OUTFIT_SYSTEM up to its corresponding #endif // MUDSKIP_OUTFIT_SYSTEM
# Pokeemerald's outfit block always ends with #endif // MUDSKIP_OUTFIT_SYSTEM EXCEPT line 52.
# Let's fix line 50-52 first.
text = text.replace('''#ifdef MUDSKIP_OUTFIT_SYSTEM
#include "outfit_menu.h"
#endif''', '')

text = re.sub(r'#ifdef MUDSKIP_OUTFIT_SYSTEM.*?#endif // MUDSKIP_OUTFIT_SYSTEM\n', '', text, flags=re.DOTALL)

# Now, Decor. I will map EXACT strings from pokeemerald new_shop.c.
...
# Use regex to safely replace GetItemPrice( with ItemId_GetPrice( EXCEPT BuyMenuGetItemPrice(
...
text = text.replace('DisplayItemMessageOnField(taskId, Shop_GetSellerMessage(SELLER_MSG_RETURN_TO_FIELD), ShowShopMenuAfterExitingBuyOrSellMenu);', 'DisplayItemMessageOnField(taskId, FONT_NORMAL, Shop_GetSellerMessage(SELLER_MSG_RETURN_TO_FIELD), ShowShopMenuAfterExitingBuyOrSellMenu);')
