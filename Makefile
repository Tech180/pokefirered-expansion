...
# wild_encounters.h is generated by a Python script from modular JSON files
WILD_ENCOUNTERS_TOOL_DIR := $(TOOLS_DIR)/wild_encounters
AUTO_GEN_TARGETS += $(DATA_SRC_SUBDIR)/wild_encounters.json $(DATA_SRC_SUBDIR)/wild_encounters.h

MISC_TOOL_DIR := $(TOOLS_DIR)/misc
AUTO_GEN_TARGETS +=  $(INCLUDE_DIRS)/constants/script_commands.h

ENCOUNTER_JSONS := $(shell find data/encounters -type f -name "*.json")

$(DATA_SRC_SUBDIR)/wild_encounters.json: $(ENCOUNTER_JSONS) scripts/build_encounters.py
	python3 scripts/build_encounters.py
