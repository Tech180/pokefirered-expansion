...
static const struct Seller sSellers[] = {
    ...
    [SELLER_JERRY] = {
        ...
        .message = {
            [SELLER_MSG_RETURN_TO_FIELD]   = sText_AnythingElseICanHelp,
...
static u8 CreateShopMenu(u8 martType)
{
    ...
    PrintMenuActionTextsAtPos(sMartInfo.windowId, FONT_NORMAL, GetMenuCursorDimensionByFont(FONT_NORMAL, 0), 2, 16, numMenuItems, sMartInfo.menuActions);
...
static void NewShop_CB2_GoToSellMenu(void)
{
    GoToBagMenu(ITEMMENULOCATION_SHOP, POCKETS_COUNT_NO_CASES, CB2_ExitSellNewShopMenu);
}

static void Task_HandleShopMenuSell(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    tCallbackHi = (u32)NewShop_CB2_GoToSellMenu >> 16;
    tCallbackLo = (u32)NewShop_CB2_GoToSellMenu;
    gTasks[taskId].func = Task_GoToBuyOrSellMenu;
    FadeScreen(FADE_TO_BLACK, 0);
}
...
static void Task_ReturnToShopMenu(u8 taskId)
{
    ...
    {
        DisplayItemMessageOnField(taskId, 1, Shop_GetSellerMessage(SELLER_MSG_RETURN_TO_FIELD), ShowShopMenuAfterExitingBuyOrSellMenu);
    }
}
...
static void PrintMoneyLocal(u8 windowId, u32 x, u32 y, u32 amount, u8 colorIdx, u32 align, bool32 copy)
{
    ...
    u32 numDigits = 1; u32 temp = amount; while(temp /= 10) numDigits++;
...
static void BuyMenuDrawGraphics(void)
{
    if (IsMartTypeCoin(sMartInfo.martType))
        ...
        PrintMoneyLocal(WIN_MONEY, RIGHT_ALIGNED_X, 0, 0, COLORID_NORMAL, STR_CONV_MODE_RIGHT_ALIGN, TRUE);
    else // if (IsMartTypeMoney(sMartInfo.martType))
...
static void UpdateItemData(void)
{
    ...
    if (sMartInfo.itemList[GridMenu_SelectedIndex(sShopData->gridItems)] == ITEM_NONE)
    {
        ...
        BuyMenuPrint(WIN_ITEM_DESCRIPTION, sText_QuitShopping, 8, 0, TEXT_SKIP_DRAW, COLORID_BLACK, TRUE);
    }
    else
    {
...
static void Task_BuyMenuTryBuyingItem(u8 taskId)
{
    ...
    else if ((GetCoins() < sShopData->totalCost) && IsMartTypeCoin(sMartInfo.martType))
    {
        PlaySE(SE_BOO);
        str = Shop_GetSellerMessage(SELLER_MSG_BUY_FAIL_NO_COINS);
        BuyMenuDisplayMessage(taskId, str, Task_ReturnToItemListWaitMsg);
    }
    else if ((0) && IsMartTypePoints(sMartInfo.martType))
    {
        PlaySE(SE_BOO);
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    if (sShopData->totalCost == 0)
        ...
        maxQuantity = 0 / sShopData->totalCost;
    else // if (IsMartTypeMoney(sMartInfo.martType))
...
void NewShop_CreatePokemartMenuFromId(enum ShopID shopId)
{
    NewShop_CreatePokemartMenu(GetShopItems(shopId));
}
