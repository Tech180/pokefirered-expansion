...
static void DexNavGenerateMoveset(enum Species species, u8 searchLevel, u8 encounterLevel, enum Move *moveDst);
...
static bool8 CapturedAllHiddenMons(u32 headerId)
{
    ...
    GetSeasonAndTimeOfDayForEncounters(headerId, WILD_AREA_HIDDEN, &season, &timeOfDay);
    hiddenMonsInfo = gWildMonHeaders[headerId].encounterTypes[season][timeOfDay].hiddenMonsInfo;
    ...
    if ((*stepPtr) == 0 && (Random() % 100 < HIDDEN_MON_SEARCH_RATE))
    {
        ...
        case 1: // water
            if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING))
            {
                ...
                {
                    species = gWildMonHeaders[headerId].encounterTypes[season][timeOfDay].waterMonsInfo->wildPokemon[ChooseWildMonIndex_Water()].species;
