...
//
//  Graphic and Tilemap Pointers for Bgs and Mugshots
//
...
void Task_OpenMainMenu(u8 taskId)
{
    ...
    {   
        switch (data[0])
        {
            ...
            default:
                SetMainCallback2(StartNewGameScene);
                DestroyTask(taskId);
                return;
            case HAS_SAVED_GAME:       
...
static void MainMenu_FreeResources(void)
{
    TRY_FREE_AND_SET_NULL(sMainMenuDataPtr);
...
static void Task_MainMenuMain(u8 taskId)
{
    if (JOY_NEW(A_BUTTON)) // If Pressed A go to thing you pressed A on
    {
        ...
        switch(sSelectedOption)
        {
            ...
            case HW_WIN_NEW_GAME:
                sMainMenuDataPtr->savedCallback = StartNewGameScene;
                sSelectedOption = HW_WIN_CONTINUE;
                break;
            case HW_WIN_OPTIONS:
    ...
    if(JOY_NEW(DPAD_DOWN)) // Handle DPad directions
    {
        switch (menuType)
        {
            case HAS_SAVED_GAME:
